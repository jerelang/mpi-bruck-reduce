...
int main(int argc, char *argv[]) {
  ...
  for (int r = 0; r < warmup + repeat; ++r) {
    ...
    switch (algorithm) {
    case 0:
      Baseline(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, recvbuf.data(), msg_size, MPI_INT,
               MPI_COMM_WORLD);
      break;
    case 1:
      Dissemination(sendbuf.data(), msg_size, MPI_INT, recvbuf.data(), msg_size, MPI_INT,
                    MPI_COMM_WORLD);
      break;
    case 2:
      Circulant(sendbuf.data(), msg_size, MPI_INT, recvbuf.data(), msg_size, MPI_INT,
                MPI_COMM_WORLD);
      break;
    }
  ...
  if (rank == 0) {
    std::cout << "Algorithm: " << algorithm << ", Msg size: " << msg_size << ", Avg time over "
              << repeat << " runs: " << global_avg * 1e6 << " us\n";
  }
  ...
  if (check) {
    ...
    int err = MPI_Allgather(sendbuf.data(), msg_size, MPI_INT, reference.data(), msg_size, MPI_INT,
                            MPI_COMM_WORLD);
    if (err != MPI_SUCCESS) {
    ...
    for (int i = 0; i < msg_size * size; ++i) {
      if (recvbuf[i] != reference[i]) {
        std::cerr << "Process " << rank << ": mismatch at index " << i << " (got " << recvbuf[i]
