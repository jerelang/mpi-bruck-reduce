...
int Dissemination(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
  ...
  // Compute how many blocks and in which rounds have to be sent additionaly to
  // the partial merged lists
  ...
  for (k = 0; k < log_p; ++k) {
    ...
    // If r>0, additional blocks are sent and received, which have to be merged
    // with the local list and then stored in the partial buffer
    if (r > 0 && partial_size > 0) {
      memcpy(local + original_size, partial, partial_size * sizeof(int));
      current_size += partial_size;
    }

    MPI_Sendrecv(local, current_size, sendtype, partner_send, 0, recv_block, current_size, recvtype,
                 partner_recv, 0, comm, MPI_STATUS_IGNORE);
  ...
  if (l != 0) {
    ...
    MPI_Sendrecv(partial, partial_size, sendtype, partner_send, 0, recv_block, partial_size,
                 recvtype, partner_recv, 0, comm, MPI_STATUS_IGNORE);
